...
void baseRateTask(void *arg)
{
  ...
  while ((rtmGetErrorStatus(Rasp_Uart_M) == (NULL)) && !rtmGetStopRequested
         (Rasp_Uart_M) ) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(Rasp_Uart_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(Rasp_Uart_M, true);
      }

      if (rtmGetStopRequested(Rasp_Uart_M) == true) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(Rasp_Uart_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(Rasp_Uart_M, true);
      }
    }
    ...
    {                                  /* Sample time: [0.2s, 0.0s] */
      rtExtModeUpload(0, Rasp_Uart_M->Timing.taskTime0);
    }
...
void MW_exitHandler(int sig)
{
  rtmSetErrorStatus(Rasp_Uart_M, "stopping the model");
  sem_post(&stopSem);
}
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(Rasp_Uart_M, 0);
  ...
  Rasp_Uart_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(Rasp_Uart_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(Rasp_Uart_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(Rasp_Uart_M, true);
    }
  }
  ...
  if (rtmGetErrorStatus(Rasp_Uart_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(Rasp_Uart_M));
  }
